...
const String SUPABASE_URL = "https://naadaumxaglqzucacexb.supabase.co";
...
void setup() {
  ...
  // WebSocket connection is async, need to call loop() to process events
  Serial.println("⏳ Connecting to WebSocket...");

  // Wait for connection with timeout (10 seconds)
  unsigned long connectStart = millis();
  while (millis() - connectStart < 10000) {
    realtime.loop();  // Process WebSocket events
    delay(10);

    // TODO: Add actual connection check here when available
    // For now, just process events for a bit
    if (millis() - connectStart > 2000) {
      break;  // Assume connected after 2 seconds of processing
    }
  }
...
void loop() {
  ...
  // Always process WebSocket events (needed for connection and reconnection)
  realtime.loop();
...
void reconnectWebSocket() {
  ...
  // Wait for connection with timeout (5 seconds for reconnect)
  unsigned long reconnectStart = millis();
  while (millis() - reconnectStart < 5000) {
    realtime.loop();  // Process WebSocket events
    delay(10);

    if (millis() - reconnectStart > 1000) {
      break;  // Assume connected after 1 second for reconnect
    }
  }
