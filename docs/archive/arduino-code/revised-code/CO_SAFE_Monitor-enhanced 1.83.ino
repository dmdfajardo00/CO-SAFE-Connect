...
const String SUPABASE_API_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5hYWRhdW14YWdscXp1Y2FjZXhiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjEwMzYwMzcsImV4cCI6MjA3NjYxMjAzN30.0ie3FXkPOaZxQfsx4c4GIBo9aMwj_RRSWQOdPRJ0bcY";
...
void setup() {
  ...
  if (!connected) {
    ...
    // CRITICAL: If memory is too low, restart to clear fragmentation
    if (finalHeap < 20000) {
      Serial.println("⚠️ CRITICAL: Memory too low (<20KB), restarting ESP8266...");
      delay(2000);
      ESP.restart();
    }
...
void reconnectWebSocket() {
  ...
  uint32_t heapBefore = ESP.getFreeHeap();
  Serial.printf("Free heap before reconnection: %d bytes\n", heapBefore);

  if (heapBefore < 22000) {
    Serial.println("⚠️ CRITICAL: Memory too low for reconnection, restarting ESP8266...");
    delay(2000);
    ESP.restart();
  }
  ...
  // Wait for reconnection with non-blocking yields and proper connection check
